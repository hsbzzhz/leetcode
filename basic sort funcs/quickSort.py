...
class Solution:
    ...
    def simple_partition(self, nums, left, right):
        # 挫方法
        pivot = nums[left]
        nums_temp = nums[left:right+1]
        nums = [each for each in nums_temp if each < pivot] + [pivot] + [each for each in nums_temp if each > pivot]
        print(nums)
